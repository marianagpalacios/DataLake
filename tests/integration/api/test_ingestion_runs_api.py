...
def test_ingestion_runs_support_filters_and_pagination(
    ...
    by_file = api_client.get(f"/api/v1/ingestion-runs?source_file_id={file_id}").json()
    by_date = api_client.get(
        "/api/v1/ingestion-runs"
        "?started_from=2026-02-01T00:00:00Z"
        "&started_to=2026-02-28T23:59:59Z"
    ).json()
    paged = api_client.get("/api/v1/ingestion-runs?page=2&size=1").json()

    assert by_status["meta"]["total"] == 1
    assert by_file["meta"]["total"] == 1
    assert by_date["meta"]["total"] == 1
    assert by_date["items"][0]["run_uuid"] == (ingestion_data["second_run_uuid"])
...
def test_ingestion_run_detail_hides_internal_fields(
    ...
    response = api_client.get(
        f"/api/v1/ingestion-runs/{ingestion_data['first_run_uuid']}"
    )
...
def test_ingestion_run_rejects_invalid_or_missing_uuid(
    ...
    invalid = api_client.get("/api/v1/ingestion-runs/not-a-uuid")
    missing = api_client.get(f"/api/v1/ingestion-runs/{uuid4()}")
...
def test_run_records_support_status_and_pagination(
    ...
    path = f"/api/v1/ingestion-runs/{ingestion_data['first_run_uuid']}/records"
    valid = api_client.get(f"{path}?validation_status=valid").json()
    rejected = api_client.get(f"{path}?validation_status=rejected").json()
    paged = api_client.get(f"{path}?page=2&size=1").json()

    assert valid["meta"]["total"] == 1
    assert valid["items"][0]["validation_status"] == "valid"
    assert rejected["meta"]["total"] == 2
    assert all(item["validation_status"] == "rejected" for item in rejected["items"])
    assert paged["meta"]["page"] == 2
...
def test_records_for_missing_run_return_404(
    ...
    response = api_client.get(f"/api/v1/ingestion-runs/{uuid4()}/records")
...
def test_quality_issues_support_filters_and_pagination(
    ...
    path = f"/api/v1/ingestion-runs/{ingestion_data['first_run_uuid']}/quality-issues"
    by_code = api_client.get(f"{path}?code=invalid_date_format").json()
    by_field = api_client.get(f"{path}?field=external_code").json()
    paged = api_client.get(f"{path}?page=2&size=1").json()
